...
impl<'a> Client<'a> {
     ...
     * Constructor
     */
    pub fn new(win: xlib::Window, wa: &xlib::XWindowAttributes, monindex: usize) -> Client<'a> {
        let c = Client {
            name: "",
            mina: 0.0, maxa: 0.0,
     ...
     */
    pub fn setfullscreen(&mut self, dpy: &mut xlib::Display, fullscreen: bool, netatom: &Vec<xlib::Atom>) {
     ...
     */
    pub fn configure(&mut self, dpy: &mut xlib::Display) {
        let ce = xlib::XConfigureEvent {
            ...
            border_width: self.bw as i32,
            above: 0,
            override_redirect: 0
        };
        unsafe { xlib::XSendEvent(dpy, self.win, 0, xlib::StructureNotifyMask, &mut xlib::XEvent { configure: ce }) };
    }
